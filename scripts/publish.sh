...
if git diff --cached --quiet; then
  notify_discord "ℹ️ New Jersey Pay Hub [$TODAY]: no changes ($ACTIVE_COUNT active)"
  exit 0
fi

git commit -m "data: NJ daily update $TODAY (+$NEW_TODAY new, $ACTIVE_COUNT active)"
git remote get-url origin && git push origin main || true

export PATH="/Users/clawii/.npm-global/bin:$PATH"
wrangler pages deploy . --project-name nj-payhub --branch main 2>&1 | tail -3 || true
...
notify_discord "✅ New Jersey Pay Hub updated [$TODAY]
