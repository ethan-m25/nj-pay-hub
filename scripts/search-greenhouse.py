#!/usr/bin/env python3
"""
nj-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — New Jersey edition.
...
Run: python3 ~/nj-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/nj-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/nj-pay-hub/scripts/.greenhouse.lock")
...
def is_nj_job(title: str, location: str, content: str) -> bool:
...
def fetch_company_jobs(slug: str, company_name_override=None):
    ...
    for j in jobs_raw:
        ...
        if not is_nj_job(title, location, content_text):
            continue
...
def main():
    ...
    log(f"=== Done. {new_count} new NJ jobs written to {OUTPUT_FILE} ===")
