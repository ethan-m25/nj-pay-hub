#!/usr/bin/env python3
"""
nj-pay-hub/scripts/search-lever.py
Lever job board scraper — New Jersey edition.
...
Run: python3 ~/nj-pay-hub/scripts/search-lever.py
"""
...
LOG_FILE  = os.path.expanduser("~/nj-pay-hub/scripts/lever.log")
LOCK_FILE = os.path.expanduser("~/nj-pay-hub/scripts/.lever.lock")
...
SEED_SLUGS = load_lever_seeds('nj')
    "workwave",  # auto-discovered 2026-05-28 — 15 NJ+salary
    "terrascend",  # auto-discovered 2026-05-28 — 4 NJ+salary
SEED_SLUGS = list(dict.fromkeys(SEED_SLUGS))


DISCOVERY_QUERIES = [
    'site:jobs.lever.co "New Jersey" OR "Newark" OR "Jersey City" OR "Trenton" salary range 2025 2026',
    'site:jobs.lever.co "New Jersey" OR "Newark" OR "Jersey City" OR "Trenton" engineer OR analyst OR manager salary',
    'site:jobs.lever.co "New Jersey" OR "Newark" OR "Jersey City" OR "Trenton" "pay range" 2025 2026',
]
...
def is_nj_location(location_str: str, desc_text: str = "") -> bool:
...
def main():
    ...
    for slug in all_slugs:
        ...
        for job in jobs:
            ...
            if not is_nj_location(loc_name, desc_plain):
                continue
            region_count += 1
        ...
        log(f"  NJ: {region_count} | New w/ salary: {found_this}")
    ...
    if newly_qualified:
        ...
        try:
            ...
            for slug, count in sorted(newly_qualified.items(), key=lambda x: -x[1]):
                if f'"{slug}"' in source:
                    continue
                log(f"  + {slug} ({count} NJ+salary jobs)")
                new_lines.append(f'    "{slug}",  # auto-discovered {TODAY} — {count} NJ+salary')
            if new_lines:
                insert_block = "\n".join(new_lines)
                marker = "\nSEED_SLUGS = list(dict.fromkeys(SEED_SLUGS))"
                if marker in source:
