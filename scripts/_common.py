#!/usr/bin/env python3
"""
nj-pay-hub/scripts/_common.py
Shared utilities for all New Jersey Pay Hub search scripts.
NJ Pay Transparency Act (P.L. 2023, c.16) — effective Jun 1, 2025 — employers with 10+ employees must include pay range + benefits description
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"nj-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/nj-pay-hub/data/jobs.json")
CURRENCY    = "USD"
REGION      = "NJ"
...
REGION_TERMS = ['new jersey', 'newark', 'jersey city', 'hoboken', 'trenton', 'camden', 'paterson', 'elizabeth', 'edison', 'woodbridge', 'teaneck', 'hackensack', 'princeton', 'parsippany', 'morristown', 'short hills', 'summit', 'westfield', 'ridgefield', 'mahwah', 'bedminster', 'basking ridge', 'florham park', 'new brunswick', ', nj', 'nj,']

_NON_REGION_LOC_TERMS = ['toronto', 'vancouver', 'ontario, canada', 'british columbia', 'canada,', 'washington, dc', 'washington dc', 'district of columbia', 'denver, co', 'boulder, co', 'aurora, co', 'seattle, wa', 'bellevue, wa', 'washington state', 'new york city', 'new york, ny', 'manhattan, ny', 'san francisco, ca', 'los angeles, ca', 'chicago, il', 'boston, ma', 'minneapolis, mn', 'burlington, vt', 'baltimore, md', 'columbia, md']
...
EXTRACT_PROMPT = """\
Extract ONE New Jersey job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid New Jersey job with explicit USD salary range is found:
{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"New Jersey location","source_url":"{url}","posted":"YYYY-MM-DD"}
...
- location must be in New Jersey
