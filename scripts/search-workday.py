#!/usr/bin/env python3
"""
nj-pay-hub/scripts/search-workday.py
Workday API scraper — New Jersey edition.

NJ Pay Transparency Act (effective Jun 1, 2025)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract New Jersey employers
  2. CXS JSON API   — paginate all jobs per employer, filter New Jersey (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/nj-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/nj-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/nj-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "New Jersey"
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "New Jersey" salary 2026',
    'site:myworkdayjobs.com "Newark" OR "Jersey City" salary "$" 2026',
    'site:myworkdayjobs.com "New Jersey" pharma OR biotech OR financial salary 2026',
    'site:myworkdayjobs.com "Parsippany" OR "Morris Plains" OR "Princeton" salary 2026',
    'site:myworkdayjobs.com "Edison" OR "Trenton" OR "Camden" NJ salary 2026',
]
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_new_jersey(p.get("locationsText", ""), p.get("externalPath", "")):
                    nj_jobs.append(p)
            offset += limit
        ...
        log(f"  NJ jobs: {len(nj_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(nj_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_new_jersey(location, external_path):
            continue
